...
import Logo from '../assets/The Ghost Strategies.png'
...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ${
      isSolid 
        ? 'backdrop-blur-xl bg-black/95 border-b border-white/20 shadow-2xl shadow-black/20' 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden lg:block">
              ...
              className="relative inline-flex items-center px-8 py-3 overflow-hidden text-sm font-semibold text-black bg-white rounded-full shadow-lg hover:shadow-xl group transition-all duration-300 cursor-pointer"
              style={{ fontFamily: 'Montserrat, sans-serif' }}
            >
              <span className="absolute inset-0 w-full h-full bg-gray-200 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></span>
            ...
            <button
              ...
              className={`relative p-3 backdrop-blur-sm rounded-xl shadow-md hover:shadow-lg transition-all duration-300 ${
                isSolid 
                  ...
                  : 'text-white hover:text-gray-200 bg-white/20 hover:bg-white/30'
              }`}
      ...
      <div className={`lg:hidden transition-all duration-500 ease-out ${
        isOpen 
          ...
          : 'max-h-0 opacity-0 invisible'
            ...
            <div className="mx-4 mb-4 rounded-2xl backdrop-blur-xl bg-black/95 border border-white/20 shadow-2xl shadow-black/20">
          <div className="px-6 py-6 space-y-2">
            {navItems.map((item, index) => {
              ...
              if (item.name === 'About Us') {
                return (
                  <Link
                    ...
                    className="flex items-center w-full px-4 py-3 font-medium rounded-xl transition-all duration-300 text-white/90 hover:text-white hover:bg-white/10 cursor-pointer"
              ...
              <button
                key={item.name}
                onClick={(e) => handleSmoothScroll(e, item.href)}
                className="flex items-center w-full px-4 py-3 font-medium rounded-xl transition-all duration-300 group text-white/90 hover:text-white hover:bg-white/10 cursor-pointer text-left"
                style={{ fontFamily: 'Montserrat, sans-serif', animationDelay: `${index * 50}ms` }}
              >
                <span className="ml-3">{item.name}</span>
            ...
            <div className="pt-4 border-t border-white/20">
              <button
                onClick={(e) => handleSmoothScroll(e, '#contact')}
                className="flex items-center justify-center w-full px-6 py-4 text-black font-semibold bg-white rounded-xl shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all duration-300 group cursor-pointer"
                style={{ fontFamily: 'Montserrat, sans-serif' }}
              >
                <span>Get Started</span>
