...
export default function AboutUs() {
  ...
  return (
    <section ref={sectionRef} id="about" className="relative pt-20 bg-white overflow-hidden">
      {/* Background Pattern */}
      <div className="absolute inset-0 opacity-5">
        <div className="absolute inset-0" style={{
          backgroundImage: `radial-gradient(circle at 25% 25%, #000000 0%, transparent 50%), 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="space-y-8">
            ...
            <div className="text-base text-gray-600 leading-relaxed max-w-2xl text-justify" style={{ fontFamily: 'Montserrat, sans-serif' }}>
              <p>
                With over 8 years of experience in digital services, The Ghost Strategies helps businesses grow through modern, creative, and result-driven digital solutions.{' '}
                <a 
                  href="/AboutUs" 
                  className="text-black hover:text-gray-700 font-semibold transition-colors duration-200 underline decoration-2 underline-offset-2 hover:decoration-gray-400"
                >
                  Read More
                </a>
              </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 pt-8 pb-5">
              {stats.map((item, idx) => (
                <div key={idx} className="text-center group bg-white border border-black/20 rounded-xl p-6 shadow-sm hover:shadow-md hover:border-black transition-all duration-300">
                  <div className="relative">
                    <div className="text-3xl font-bold text-black mb-2" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                        <Counter target={item.target} start={start} postfix={item.postfix} />
                    </div>
                  </div>
                  <div className="text-sm text-gray-600 font-medium group-hover:text-black transition-colors duration-300" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                      {item.title}
                  </div>
                </div>
              ))}
